...
- On macOS, run `node ../../scripts/apply-wallpaper.mjs /absolute/path/to/final.png` from this skill directory when the user authorized desktop replacement. Treat the JSON field `verified: true` as success; otherwise keep the PNG and report the failure without claiming the desktop changed.
